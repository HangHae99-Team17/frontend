...
const SaleWrite = (props) => {
  ...
  const _sale = editmode
    ? sale_list.find((Notice) => Notice.id.toString() === sale_id)
    : null;
  ...
  const [startDate, setStartDate] = useState(
    _sale ? new Date(_sale.couponCreate) : new Date()
  );
  const [endDate, setEndDate] = useState(
    _sale ? new Date(_sale.couponDespire) : new Date()
  );
